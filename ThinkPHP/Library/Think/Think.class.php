...
class Think {
    ...
    static public function start() {
      ...
      // 初始化文件存储方式
        ///上面调用Log类自动注册1次，Storage调用一次，conncet方法有调用一次
      ...
      if(!APP_DEBUG && Storage::has($runtimefile)){
          ...
          // 加载应用模式配置文件
          ///惯用配置convertion和自定义配置文件，有相同的key进行合并?--返回值为Null但是已存入static静态数组中
          ...
          // 加载框架底层语言包
          ///上面加载了惯用配置文件DEFAULT_LANG被存在C函数的静态变量“数组”中---直接读取其对应的值
      ...
      // 检查应用目录结构 如果不存在则自动创建
        ///is_dir判断目录是否存在--调用Build--循环创建目录--在创建控制器--model等---默认HOME模块
      if(C('CHECK_APP_DIR')) {
      ...
      G('loadTime');
      // 运行应用
        ///D:\phpstudy\PHPTutorial\WWW\tp\ThinkPHP\Library\Think\App.class.php
      App::run();
    }
    ...
    static public function fatalError() {
        Log::save();
        ///使用error_get_lasdt()函数获取最后发生的错误信息https://baike.baidu.com/item/PHP%20error_get_last/1309051?fr=aladdin
        if ($e = error_get_last()) {
            switch($e['type']){
              case E_ERROR:
              case E_PARSE:
